...
import {
  Crown, Zap, UserCircle, Loader2,
  BookOpen, Briefcase, Star, Building2, MousePointer2, Check
} from "lucide-react";
...
const PLANS = [
  ...
  { id: "basic", title: "Basic", price: 955, icon: MousePointer2, color: "text-emerald-400", features: ["Até 2 downloads de CV por mês", "Perfil profissional com 2 links clicáveis", "Acesso aos recursos essenciais"] },
  { id: "starter", title: "Starter", price: 1699, icon: Zap, color: "text-sky-400", features: ["Até 4 downloads de CV por mês", "Perfil aprimorado com 3 links clicáveis", "Ferramentas iniciais de gestão"] },
  { id: "student", title: "Student", price: 2000, icon: BookOpen, color: "text-orange-400", features: ["Até 6 downloads de CV por mês", "Interface melhorada e 5 links clicáveis", "Trabalhos escolares (6 pág / 4 transf. mês)", "2 Layouts profissionais para trabalhos"] },
  { id: "student_pro", title: "Student Pro", price: 2895, icon: Star, color: "text-yellow-400", features: ["Até 10 downloads de CV por mês", "Perfil sofisticado (6 links + fundo custom)", "Trabalhos (15 pág / transf. ilimitadas)", "5 Layouts profissionais para trabalhos"] },
  { id: "professional", title: "Professional", price: 3955, icon: Briefcase, color: "text-indigo-400", features: ["8 downloads de CV a cada 2 semanas", "Interface profissional com 8 links clicáveis", "Ferramentas avançadas de apresentação"] },
  { id: "professional_pro", title: "Professional Pro", price: 5775, icon: Crown, color: "text-rose-400", features: ["8 downloads de CV por semana", "Premium (10 links + fundo custom)", "Venda de tickets (6 eventos / 5.000 un.)", "Cartas e Contratos (8 downloads/semana)", "Portfólio profissional"] },
  { id: "business", title: "Business", price: 10679, icon: Building2, color: "text-purple-400", features: ["Downloads ILIMITADOS (CV e Docs)", "Links ilimitados + Vídeo de fundo", "Eventos e Ingressos SEM LIMITES", "Análises avançadas e Chat com clientes", "Selo de Verificação Corporativa"] }
];

export default function Billing_Gpayment({ country }: { country: string }) {
  ...
  const loadBillingData = useCallback(async () => {
    try {
      ...
      setSelectedPlanId(isPlanActive && profileData.role ? profileData.role.toLowerCase() : "free");
  ...
  const handleCheckout = useCallback(async () => {
    ...
    try {
      setProcessing(true);
      const { data } = await api.post("/gpayment/create-payment/", {
        plan_id: selectedPlan.id,
        payment_method: "multicaixa"   // Usa Multicaixa Express (redirect)
      });

      if (data.status === "redirect" && data.payment_url) {
        // Redireciona para o gateway de pagamento
        window.location.href = data.payment_url;
      } else if (data.status === "pending" && data.reference_number) {
        // Caso a resposta seja do tipo referência (fallback)
        setAlert({
          message: `Pagamento por referência gerado. Entidade: ${data.entity}, Referência: ${data.reference_number}, Vencimento: ${new Date(data.due_date).toLocaleString()}`,
          type: "success"
        });
      } else {
      ...
      const msg = err.response?.data?.error || err.message || "Erro ao iniciar pagamento";
      setAlert({ message: msg, type: "error" });
    } finally {
      setProcessing(false);
    }
  }, [selectedPlan, isSelectedPlanActive]);

  if (loading) return <LoadingPage />;

  const summaryCardClasses = "p-8 rounded-3xl border border-foreground/10 bg-foreground/5 backdrop-blur-md";

  return (
    <div className="min-h-screen bg-background text-foreground p-6">
      {alert && <ImlinkeyAlert message={alert.message} type={alert.type} duration={3000} onClose={() => setAlert(null)} />}
