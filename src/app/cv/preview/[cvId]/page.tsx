...
export default function PreviewPage() {
  ...
  // Estado para o Toast de 6 segundos
  ...
  useEffect(() => {
    async function loadCV() {
      try {
        ...
        // Fallback para local storage se a API falhar
  ...
  const showFeedback = (msg: string, upgrade: boolean = false) => {
    ...
    <div className="min-h-screen bg-muted/20 p-4 md:p-12 relative overflow-x-hidden">
      ...
      <div className="max-w-[210mm] mx-auto mb-12 flex flex-col sm:flex-row justify-between items-center gap-6 print:hidden">
        ...
        <div className="flex items-center gap-4">
          {/* BOTÃO NATIVO DO NAVEGADOR (SEM BACKEND) */}
          <button
              ...
              {showDownloadMenu && (
                ...
                >
                  {downloadOptions.map((opt) => (
                    <button key={opt.id} onClick={() => handleDownload(opt.id)}
                    ...
                    >
                      <opt.icon size={14} className="text-primary" />
                      {opt.label}
                    </button>
                  ))}
                </motion.div>
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            <div className="bg-card border border-border p-5 rounded-2xl shadow-2xl flex flex-col gap-4">
              <div className="flex items-start gap-3">
                <div className="p-2 bg-red-500/10 rounded-lg shrink-0">
                  <AlertCircle className="text-red-500 w-5 h-5" />
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-muted-foreground font-black text-[8px] uppercase tracking-[0.2em]">Notificação do Sistema</span>
              ...
              <motion.div 
                ...
                className="h-[2px] bg-primary/30 absolute bottom-0 left-0 rounded-b-2xl"
              />
            </div>
          </motion.div>
        )}
